...
func newIdentityProvidersTestRealm(t *testing.T) (*keycloakapi.KeycloakClient, string) {
	t.Helper()
...
func TestIdentityProvidersClient_CreateIdentityProvider(t *testing.T) {
	...
	resp, err := c.IdentityProviders.CreateIdentityProvider(ctx, realmName, keycloakapi.IdentityProviderRepresentation{
		Alias:       &alias,
...
func TestIdentityProvidersClient_CreateIdentityProvider_Conflict(t *testing.T) {
	...
	idp := keycloakapi.IdentityProviderRepresentation{
		Alias:       &alias,
		DisplayName: &displayName,
		ProviderId:  &providerID,
		Enabled:     &enabled,
	...
	_, err = c.IdentityProviders.CreateIdentityProvider(ctx, realmName, idp)
	require.Error(t, err)
	require.True(t, keycloakapi.IsConflict(err), "expected 409 conflict for duplicate alias")
}
...
func TestIdentityProvidersClient_GetAndDeleteIdentityProvider(t *testing.T) {
	...
	require.True(t, keycloakapi.IsNotFound(err), "identity provider should be gone after deletion")
}
...
func TestIdentityProvidersClient_GetIdentityProvider_NotFound(t *testing.T) {
	...
	require.Error(t, err)
	require.True(t, keycloakapi.IsNotFound(err), "Should return 404 for non-existent identity provider")
	require.NotNil(t, resp)
}
...
func TestIdentityProvidersClient_UpdateIdentityProvider(t *testing.T) {
	...
	_, err = c.IdentityProviders.UpdateIdentityProvider(ctx, realmName, alias, keycloakapi.IdentityProviderRepresentation{
		Alias:       &alias,
		ProviderId:  &providerID,
		Enabled:     &enabled,
		DisplayName: &displayName,
...
func TestIdentityProvidersClient_Mappers(t *testing.T) {
	...
	_, err = c.IdentityProviders.CreateIDPMapper(ctx, realmName, alias, keycloakapi.IdentityProviderMapperRepresentation{
		Name:                   &mapperName,
...
func TestIdentityProvidersClient_UpdateIDPMapper(t *testing.T) {
	...
	mapperResp, err := c.IdentityProviders.CreateIDPMapper(ctx, realmName, alias,
		keycloakapi.IdentityProviderMapperRepresentation{
			Name:                   &mapperName,
			IdentityProviderAlias:  &alias,
			IdentityProviderMapper: &mapperType,
			Config:                 &mapperConfig,
	...
	mapperID := keycloakapi.GetResourceIDFromResponse(mapperResp)

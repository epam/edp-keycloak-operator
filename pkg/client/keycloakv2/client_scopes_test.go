...
func TestClientScopesClient_CRUD(t *testing.T) {
	...
	_, err = c.ClientScopes.UpdateClientScope(ctx, realmName, scopeID, keycloakv2.ClientScopeRepresentation{
		Name:        &scopeName,
		Protocol:    &protocol,
		Description: &newDesc,
	})
...
func TestClientScopesClient_ProtocolMappers(t *testing.T) {
	...
	_, err = c.ClientScopes.CreateClientScopeProtocolMapper(
		ctx, realmName, scopeID, keycloakv2.ProtocolMapperRepresentation{
			Name:           &mapperName,
...
func containsScope(scopes []keycloakv2.ClientScopeRepresentation, name string) bool {
	for _, s := range scopes {

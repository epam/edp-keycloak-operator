...
func ExampleNewKeycloakClient() {
	...
	kc, err := keycloakv2.NewKeycloakClient(ctx, "http://localhost:8080", "admin-cli",
...
func ExampleNewKeycloakClient_clientCredentials() {
	...
	_, err := keycloakv2.NewKeycloakClient(ctx, "http://localhost:8080", "my-service-account",
		keycloakv2.WithClientSecret("my-secret"),
	)
	if err != nil {
		log.Fatal(err)
	}
...
func Example_userCRUD() {
	...
	// Create a user.
	resp, err := kc.Users.CreateUser(ctx, realm, keycloakv2.UserRepresentation{
...
func Example_errorHandling() {
	...
	_, err = kc.Roles.CreateRealmRole(ctx, realm, keycloakv2.RoleRepresentation{
		Name: ptr.To("my-role"),
	})
	if err := keycloakv2.SkipConflict(err); err != nil {
...
func Example_authFlowCopyAndModify() {
	...
	if len(executions) > 0 {
		executions[0].Requirement = ptr.To(keycloakv2.AuthExecutionRequirementRequired)

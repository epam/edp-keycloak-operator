...
func TestAuthFlowsClient_UpdateAndDeleteAuthenticatorConfig(t *testing.T) {
	...
	_, err = c.AuthFlows.CreateAuthFlow(ctx, realmName, keycloakv2.AuthFlowRepresentation{
		Alias:      &flowAlias,
		TopLevel:   &topLevel,
		BuiltIn:    &builtIn,
		ProviderId: &providerID,
	...
	execResp, err := c.AuthFlows.AddExecutionToFlow(ctx, realmName, keycloakv2.AuthenticationExecutionRepresentation{
		ParentFlow:    &flowID,
	...
	executionID := keycloakv2.GetResourceIDFromResponse(execResp)
	...
	configID := keycloakv2.GetResourceIDFromResponse(cfgResp)
...
func TestAuthFlowsClient_RequiredActions(t *testing.T) {
	...
	var verifyEmail *keycloakv2.RequiredActionProviderRepresentation

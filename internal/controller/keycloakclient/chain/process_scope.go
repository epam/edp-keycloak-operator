...
func NewProcessScope(kClient *keycloakapi.KeycloakClient, k8sClient client.Client) *ProcessScope {
...
func (h *ProcessScope) Serve(ctx context.Context, keycloakClient *keycloakApi.KeycloakClient, realmName string, clientCtx *ClientContext) error {
	...
	for _, scope := range keycloakClient.Spec.Authorization.Scopes {
		...
		scopeRep := keycloakapi.ScopeRepresentation{Name: &scope}
...
func (h *ProcessScope) deleteScopes(ctx context.Context, existingScopes map[string]keycloakapi.ScopeRepresentation, realmName string, clientUUID string) error {
	log := ctrl.LoggerFrom(ctx)

...
func (m *mockHandler) Serve(_ context.Context, _ *keycloakApi.KeycloakClientScope, _ string) error {
	m.called = true
...
func TestChain_Serve_EmptyChain(t *testing.T) {
	ch := &Chain{}
	err := ch.Serve(context.Background(), &keycloakApi.KeycloakClientScope{}, testRealmName)
...
func TestMakeChain_NotNil(t *testing.T) {
	kc := &keycloakv2.KeycloakClient{
		ClientScopes: mocks.NewMockClientScopesClient(t),
	}
	ch := MakeChain(kc)
	require.NotNil(t, ch)
	require.Len(t, ch.handlers, 3)
}

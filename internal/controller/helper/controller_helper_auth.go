...
func (h *Helper) CreateKeycloakeycloakAPIClientFromRealm(ctx context.Context, realm *keycloakApi.KeycloakRealm) (*keycloakapi.APIClient, error) {
	authData, err := h.getKeycloakAuthDataFromRealm(ctx, realm)
	if err != nil {
		return nil, err
	}

	return h.createKeycloakeycloakAPIClientFromAuthData(ctx, authData)
}

func (h *Helper) CreateKeycloakeycloakAPIClientFromKeycloak(ctx context.Context, kc *keycloakApi.Keycloak) (*keycloakapi.APIClient, error) {
...
func (h *Helper) CreateKeycloakeycloakAPIClientFromClusterKeycloak(ctx context.Context, kc *keycloakAlpha.ClusterKeycloak) (*keycloakapi.APIClient, error) {
...
func (h *Helper) CreateKeycloakeycloakAPIClientFromRealmRef(ctx context.Context, object ObjectWithRealmRef) (*keycloakapi.APIClient, error) {
...
func (h *Helper) CreateKeycloakeycloakAPIClientFromClusterRealm(ctx context.Context, realm *keycloakAlpha.ClusterKeycloakRealm) (*keycloakapi.APIClient, error) {
...
func (h *Helper) createKeycloakeycloakAPIClientFromAuthData(ctx context.Context, authData *KeycloakAuthData) (*keycloakapi.APIClient, error) {
	var options []keycloakapi.ClientOption

	clientID := keycloakapi.DefaultAdminClientID
	...
	if authData.AuthSpec != nil {
		...
		if authData.AdminType == keycloakApi.KeycloakAdminTypeServiceAccount {
			...
			options = append(options, keycloakapi.WithClientSecret(password))
		} else {
			options = append(options, keycloakapi.WithPasswordGrant(username, password))
		}
	}

	if authData.CACert != "" {
		options = append(options, keycloakapi.WithCACert(authData.CACert))
	}

	if authData.InsecureSkipVerify {
		options = append(options, keycloakapi.WithTLSInsecureSkipVerify(true))
	}

	kcClient, err := keycloakapi.NewAPIClient(ctx, authData.Url, clientID, options...)
	if err != nil {
...
func (h *Helper) buildV2AuthOptions(
...
) (clientID string, options []keycloakapi.ClientOption, err error) {
	switch {
	case authData.AuthSpec.PasswordGrant != nil:
		...
		return keycloakapi.DefaultAdminClientID,
			[]keycloakapi.ClientOption{keycloakapi.WithPasswordGrant(username, password)},
			nil
	...
	case authData.AuthSpec.ClientCredentials != nil:
		...
		return clientID,
			[]keycloakapi.ClientOption{keycloakapi.WithClientSecret(clientSecret)},
			nil

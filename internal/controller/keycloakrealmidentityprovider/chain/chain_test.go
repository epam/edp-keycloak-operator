...
func (h *testHandler) Serve(_ context.Context, _ *keycloakApi.KeycloakRealmIdentityProvider, _ string) error {
	h.called = true
	return h.err
}
...
func TestChain_Serve(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := ch.Serve(ctx, &keycloakApi.KeycloakRealmIdentityProvider{}, "realm")
			tt.wantErr(t, err)
...
func TestMakeChain(t *testing.T) {
	...
	c := MakeChain(&keycloakapi.KeycloakClient{}, k8sClient)

...
### 4a. Confirm the controller uses keycloak
...
- **Field exists in keycloak representation** → proceed to Step 5.
- **Field missing from keycloak representation** → **stop**. Warn the user that the field is not exposed by the client. `openapi.yaml` is auto-generated and must not be edited manually. Ask the user how to proceed before making any further changes.
